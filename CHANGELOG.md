...
### Fixed
- **Config divergences that accumulated as features were added** (audit sweep):
  - **Bootstrap player (generated by `setup-kiosk.sh`) rotated only 3 formats
    and baked a literal `STREAM_URL`.** The hand copy of `player.sh` that runs
    between `make setup` and `make deploy` missed the GIF/WebP support added in
    0.25.0 (GIF/WebP slides were invisible until the first deploy) and ignored
    `/etc/default/kiosk`. It now globs all five formats and reads the config
    store directly (the bootstrap service has no `EnvironmentFile=`), matching
    the canonical player. The `create_splash` source picker also now offers
    GIF/WebP images.
  - **`systemctl --user` restart missing `DBUS_SESSION_BUS_ADDRESS` in three
    call sites.** The "user bus address required" fix from `become-kiosk.sh`
    (2026-06-13) had not reached `dev/deploy.sh`, the `make restart` target, or
    `setup-kiosk.sh`'s daemon-reload/enable — they set only `XDG_RUNTIME_DIR`.
    All now pass the bus address (deploy sudoers already grants SETENV).
  - **Health-staleness threshold disagreed:** the on-screen overlay
    (`mpv-health-overlay.lua`) flagged STALE after 60 s while `healthcheck.sh`
    and the web board use 120 s. Overlay raised to 120 s so all three agree.
  - **Docs:** `docs/index.html` still advertised "PNG/JPEG"; now lists all four.
