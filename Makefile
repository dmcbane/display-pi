...
.PHONY: help setup provision deploy sudoers config splash-ls test-stream test-stream-long ssh ssh-password logs status diag judder-tree judder-probe judder-monitor stream-key hdmi-mode set-time test lint check ping reboot restart shutdown setup-web setup-web-tls-local web-ca setup-web-tls volunteer-web-url
